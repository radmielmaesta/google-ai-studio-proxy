...
model = "gemini-3-pro-preview" # @param ["gemini-3-pro-preview","gemini-2.5-pro","gemini-2.5-flash","gemini-3-flash-preview","gemini-3.5-flash"]
...
def handle_proxy():
    ...
    try:
        ...
        # Base config (max tokens is still safe to pass)
        generation_config = {
            "maxOutputTokens": json_data.get('max_tokens', max_tokens)
        }

        # Only add legacy sliders if we ARE NOT using a 3.x model
        if "3" not in selected_model:
        ...
        # Strip frequency/presence penalty for 3.x models to prevent 400 errors
        if "3" not in selected_model:
            ...
            if is_streaming:
                ...
                def generate_stream():
                    ...
                    try:
                        ...
                        try:
                            response = requests.post(url, headers=headers, json=google_ai_request, stream=True, timeout = timeout_seconds)
                            print(f"Google AI stream response status: {response.status_code}")
                            response.raise_for_status()
                        except requests.exceptions.HTTPError as e:
                            print("\n🚨 GOOGLE API ERROR BODY 🚨")
                            print(e.response.text)  # This is the golden ticket
                            raise e
